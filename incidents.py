            ...
            {{
              actor {{
                account(id: {accountId}) {{
                  nrql(query: "SELECT incidentId, mutingId, policyName, conditionName FROM (SELECT uniqueCount(event) as 'total', latest(event) as 'state', latest(priority) as 'priority', latest(muted) as 'muted', latest(mutingRuleId) as 'mutingId' FROM NrAiIncident where event in ('open','close') facet incidentId, policyName, conditionName limit max) where total=1 and state='open' and muted is true limit max since 1 day ago") {{
                    results
                  }}
                }}
              }}
